...
**"This query does not support recovering from checkpoint location"** (Auto Loader): The checkpoint was created by a different query (sink/options changed or runtime upgrade). Delete the checkpoint for that object: remove `landing.checkpoint/offsets` or the whole `_checkpoint` dir (e.g. `.../accounts/_checkpoint`). Next run will re-process all files once, then resume. See plugin docstring in `file_autoloader_plugin.py`.
